...
int avgMain(uint64_t width, uint64_t height, char fileName[2000], char filePath[2000], bool print)
{
	FILE* fp;
...
//OPTIMIZATION EXPERIMENTATION BELOW:
int avgMainOpt(uint64_t width, uint64_t height, char fileName[2000], char filePath[2000], bool print)
{
	FILE* fp;
	...
	avgOutputFilePath += fileName;
	avgOutputFilePath = avgOutputFilePath.substr(0, avgOutputFilePath.find_last_of('.'));
	avgOutputFilePath += "_avgOutputOpt.yuv";
	fopen_s(&avgOutput, avgOutputFilePath.c_str(), "wb");
